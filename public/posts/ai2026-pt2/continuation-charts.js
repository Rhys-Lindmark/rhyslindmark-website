  ...
  if(scene.id==='anthropic-rd-automation'){
    ...
    const months=rows.map((row,i)=>{
      ...
      bands.forEach(({index,color,name})=>{
        const lower=row.slice(index+2).reduce((sum,value)=>sum+value,0),upper=lower+row[index+1];
        const bar=n('rect',{x:edge(i)+.5,y:y(upper),width:cell-1,height:y(lower)-y(upper),fill:color,stroke:'#0b1015','stroke-width':1},month);
        n('title',{},bar,`${row[0]} · ${name}: ${row[index+1]}%`);
      });
      return month;
    });
    ...
    n('circle',{cx:x,cy,r:small?4:5,fill:'#39ffc1',stroke:'#0b1015','stroke-width':2},months.at(-1));
    const value=text(x,cy-14,'26%','middle',months.at(-1));value.style.fill='#39ffc1';value.style.fontWeight='700';value.style.fontSize=small?'15px':'20px';
    ...
    const render=()=>{
      ...
      const visible=Math.floor(progress*months.length);
