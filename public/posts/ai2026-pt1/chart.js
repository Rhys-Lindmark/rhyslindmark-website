...
function between(points,key){if(!points?.length||key<points[0][0]||key>points.at(-1)[0])return null;let i=0;while(i<points.length-2&&points[i+1][0]<key)i++;const a=points[i],b=points[Math.min(i+1,points.length-1)],t=a===b?0:clamp((key-a[0])/(b[0]-a[0]));return lerp(a[1],b[1],t);}
function hoverX(svg,a,domain,series,formatTitle,formatValue){
 const bounds={left:a.m.l,right:a.m.l+a.iw,top:a.m.t,bottom:a.m.t+a.ih},keys=[...new Set(series.flatMap(s=>s.points.map(p=>p[0])))].sort((x,y)=>x-y),keyboard=keys.map(key=>({x:a.x(key),y:(bounds.top+bounds.bottom)/2}));
 window.AIChartHover?.attach(svg,{bounds,keyboard,get:point=>{const key=domain[0]+(point.x-bounds.left)/a.iw*(domain[1]-domain[0]),items=series.map(s=>{const value=between(s.points,key);return value===null?null:{label:s.name,value:formatValue(value,s),color:s.color,x:a.x(key),y:clamp(a.y(value),bounds.top,bounds.bottom)};}).filter(Boolean);return {title:formatTitle(key),x:a.x(key),guide:'x',items};}});
}
function hoverRows(svg,bounds,rows){
 window.AIChartHover?.attach(svg,{bounds,keyboard:rows.map(r=>({x:r.x,y:r.y})),get:point=>{const row=rows.find(item=>barContains(item.bar,point));return row?{...row,guide:'y'}:null;}});
}
function barContains(bar,point){
 if(!bar||bar.getAttribute('width')==='0'||bar.getAttribute('height')==='0'||getComputedStyle(bar).visibility==='hidden')return false;
 const local=bar.ownerSVGElement.createSVGPoint();local.x=point.x;local.y=point.y;
 const svgMatrix=bar.ownerSVGElement.getScreenCTM(),barMatrix=bar.getScreenCTM();if(!svgMatrix||!barMatrix)return false;
 const p=local.matrixTransform(svgMatrix).matrixTransform(barMatrix.inverse()),box=bar.getBBox();
 return p.x>=box.x&&p.x<=box.x+box.width&&p.y>=box.y&&p.y<=box.y+box.height;
}
/* Projections are stored as a straight two-point segment. Redraw them with the same
   month-to-month texture as the measured series, pinning both ends to the real figures. */
function wobbleProjection(pts,seed){
 if(!pts||pts.length<2)return pts;
 const [x0,y0]=pts[0],[x1,y1]=pts[pts.length-1],n=16,out=[];let r=seed>>>0;
 const noise=()=>((r=r*1664525+1013904223>>>0)/4294967296)-.5;
 for(let i=0;i<=n;i++){const t=i/n,taper=Math.sin(Math.PI*t);
  out.push([x0+(x1-x0)*t,y0+(y1-y0)*t+taper*(noise()*1.9+Math.sin(t*8.4+seed%5)*.6)]);}
 return out;
}
function drawLines(scene){const {svg,W,H,small}=svgBase(scene),investment=scene.dataset.kind==='investment',series=investment?data.investment:data.construction,xd=investment?[1852,2030]:[2014,2026.5834],yd=investment?[0,6]:[0,80];
 ...
 const hoverSeries=series.map(s=>({...s,points:investment?s.points:[...s.points,...s.projection.slice(1)]}));
 hoverX(svg,a,xd,hoverSeries,year=>investment?String(Math.round(year)):`${new Date(Date.UTC(Math.floor(year),Math.max(0,Math.min(11,Math.round((year%1)*12))),1)).toLocaleDateString('en-US',{month:'short',year:'numeric',timeZone:'UTC'})}${year>2025.398?' · estimate':''}`,value=>investment?`${value.toFixed(2)}% of GDP`:`$${value.toFixed(1)}B`);
...
function drawChips(scene){const {svg,W,H,small}=svgBase(scene),q=data.chips.quarters,groups=data.chips.groups,a=axes(svg,W,H,{xd:[0,q.length],yd:[0,26],xt:small?[.5,4.5,8.5]:q.map((_,i)=>i+.5),yt:[0,5,10,15,20,25],xfmt:v=>q[Math.floor(v)].label,yfmt:v=>`${v}M`,yTitle:'CUMULATIVE COMPUTE · MILLION H100e',xTitle:'QUARTER'}),c=clipping(svg,'clip-chips',a.m,a.ih),pat=node('pattern',{id:'incomplete',width:6,height:6,patternUnits:'userSpaceOnUse',patternTransform:'rotate(35)'},c.defs);node('line',{x1:0,x2:0,y1:0,y2:6,stroke:'#cbdce8','stroke-width':2,opacity:.55},pat);
 ...
 const bounds={left:a.m.l,right:a.m.l+a.iw,top:a.m.t,bottom:a.m.t+a.ih};window.AIChartHover?.attach(svg,{bounds,keyboard:q.map((_,i)=>({x:a.x(i+.5),y:(bounds.top+bounds.bottom)/2})),get:point=>{const j=clamp(Math.floor((point.x-a.m.l)/a.iw*q.length),0,q.length-1),quarter=q[j];let bottom=0;const items=[];quarter.values.forEach((v,i)=>{if(!v)return;items.push({label:groups[i].name,value:`${v.toFixed(2)}M H100e`,color:groups[i].color,x:a.x(j+.5),y:a.y(bottom+v/2)});bottom+=v;});return{title:quarter.label,x:a.x(j+.5),items};}});
...
function drawProfit(scene){const {svg,W,H,small}=svgBase(scene),margin=scene.dataset.kind==='margin',metric=margin?'margin':'profit',rows=data.profit.filter(d=>d.name!=='IBM'),left=small?82:130,right=small?55:100,top=32,bottom=36,iw=W-left-right,rh=(H-top-bottom)/rows.length,domain=margin?[0,80]:[0,290],x=v=>left+(v-domain[0])/(domain[1]-domain[0])*iw;
 ...
 const bar=node('rect',{x:x(0),y:y+rh*.15,width:0,height:rh*.68,fill:color,'fill-opacity':.9,stroke:color,'stroke-width':.7},svg);node('title',{},bar,`${d.name}: ${val}${margin?'%':' billion dollars'}`);const txt=label(svg,val<0?x(val)+4:x(val)+6,y+rh*.58,`${val<0?'−':''}${margin?'':'$'}${Math.abs(val).toFixed(1)}${margin?'%':'B'}`,'start','value');bars.push({bar,txt,val,d,color});});
 label(svg,left+iw/2,H-2,margin?'OPERATING MARGIN (%)':'ANNUALIZED OPERATING PROFIT ($B)','middle','axis-title');legend(scene,[]);
 hoverRows(svg,{left,right:W-right,top,bottom:H-bottom},bars.map(({bar,val,d,color})=>({bar,title:margin?'Operating margin':'Annualized operating profit',x:x(val/2),y:Number(bar.getAttribute('y'))+Number(bar.getAttribute('height'))/2,items:[{label:d.name,value:margin?`${val.toFixed(1)}%`:`${val<0?'−':''}$${Math.abs(val).toFixed(1)}B`,color,x:x(val),y:Number(bar.getAttribute('y'))+Number(bar.getAttribute('height'))/2}]})));
...
function drawEras(scene){
 ...
 const eraBounds={left:a.m.l,right:a.m.l+a.iw,top:a.m.t,bottom:a.m.t+a.ih},eraYears=Array.from({length:57},(_,i)=>1970+i);window.AIChartHover?.attach(svg,{bounds:eraBounds,keyboard:eraYears.map(year=>({x:a.x(year),y:(eraBounds.top+eraBounds.bottom)/2})),get:point=>{const year=Math.round(1970+(point.x-a.m.l)/a.iw*56),items=[];for(const s of data.eras){const profit=between(s.data.map(p=>[p.year,p.profit]),year);if(profit===null)continue;items.push({label:`${s.id} profit`,value:`$${profit.toFixed(1)}B`,color:s.color,x:a.x(year),y:a.y(profit)});items.push({label:`${s.id} avg. margin`,value:`${s.avgOperatingMargin}%`,color:s.color,x:a.x(year),y:ym(s.avgOperatingMargin)});}return{title:String(year),x:a.x(year),items};}});
 return p=>{
...
function drawMarketcap(scene){
 ...
 legend(scene,[]);
 hoverX(svg,a,xd,[{name:'NVIDIA',color:'#63ff91',points}],date=>new Date(date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'}),value=>value<.001?`$${Math.round(value*1e6)}M`:value<1?`$${Math.round(value*1000)}B`:`$${value.toFixed(2)}T`);
...
function drawElectricity(scene){
 ...
 hoverX(svg,a,[2014,2040],data.electricity.series.map(s=>({...s,points:[...s.points,[2040,s.target]]})),year=>`${Math.round(year)}${year>2025?' · estimate':''}`,value=>`${value.toFixed(2)} TW`);
...
function drawCenters(scene){
 ...
 legend(scene,[]);
 const centerKeys=items.map((_,i)=>({x:left,y:top+(i+.49)*rh}));
 window.AIChartHover?.attach(svg,{bounds:{left,right:W-right,top,bottom:H-bottom},keyboard:centerKeys,get:point=>{const item=items.find(entry=>barContains(entry.bar,point));if(!item)return null;const box=item.bar.getBBox(),offset=Number(item.g.getAttribute('transform')?.match(/translate\(0 ([^)]+)\)/)?.[1]||0),value=Number(item.bar.getAttribute('width'))/iw*max,px=box.x+box.width,py=box.y+box.height/2+offset;return{title:scene.querySelector('.readout').textContent,x:px,y:py,guide:'y',items:[{label:item.s.name,value:`${value.toFixed(2)} GW`,color:'#35e7ff',x:px,y:py}]};}});
...
function drawShare(scene){
 ...
 const shareBounds={left:m.l,right:W-m.r,top:m.t,bottom:m.t+ih},hover=window.AIChartHover?.attach(svg,{bounds:shareBounds,keyboard:d.quarters.map((_,j)=>({x:m.l+(j+.5)*iw/9,y:m.t+ih/2})),get:point=>{if(scene.dataset.currentStep==='34')return null;const j=clamp(Math.floor((point.x-m.l)/iw*9),0,8),q=d.quarters[j];let bottom=0;const items=[];q.values.forEach((v,i)=>{if(v>=.05)items.push({label:d.groups[i].name,value:`${v.toFixed(1)}%`,color:d.groups[i].color,x:m.l+(j+.5)*iw/9,y:y(bottom+v/2)});bottom+=v;});return{title:q.label,x:m.l+(j+.5)*iw/9,items};}});
 return p=>{const {index,local}=passageStage(scene,p);if(index)hover?.hide();const extend=all?1:index?clamp(local/.6):0,count=9+36*extend,bw=iw/count,reveal=all||index?9:Math.max(1,Math.ceil(local/.8*9));for(const {b,j} of bars){b.setAttribute('x',m.l+j*bw+1);b.setAttribute('width',Math.max(0,bw-2));b.style.visibility=j<reveal?'visible':'hidden';}ticks.forEach((t,j)=>{t.setAttribute('x',m.l+(j+.5)*bw);t.style.visibility=extend===0&&j<reveal&&(!small||j%4===0)?'visible':'hidden';});future.setAttribute('x',m.l+9*bw+4);future.setAttribute('width',Math.max(0,36*extend*bw-4));future.style.visibility=extend>0?'visible':'hidden';boundaryYear.setAttribute('x',m.l+9*bw);for(const t of [startYear,boundaryYear,endYear])t.style.visibility=extend>0?'visible':'hidden';revealLegend(scene,i=>d.quarters.some((q,j)=>j<reveal&&q.values[i]>0));scene.querySelector('.readout').textContent=index?'Future share is open':d.quarters[reveal-1].label;};
}
function drawUnits(scene){
 ...
 for(let i=0;i<100;i++){
  ...
  node('path',{d:'M27 16H73L84 27V73L73 84H27L16 73V27Z',fill:'#63ff91','fill-opacity':.15},chip);
  let pins='';for(const p of [30,43,57,70])pins+=`M${p} 5V16M${p} 84V95M5 ${p}H16M84 ${p}H95`;
  node('path',{d:pins,fill:'none','stroke-linecap':'round'},chip);
  for(const [cx,cy] of [[27,27],[73,27],[27,73],[73,73]])node('circle',{cx,cy,r:2.8,fill:'none','stroke-width':2},chip);
  units.push(chip);
 }
...
function drawROI(scene){
 ...
 hoverX(svg,a,[1995,2040],[{name:'Internet infrastructure',color:'#35e7ff',points:d.internetInfra},{name:'Internet revenue',color:'#35e7ff',points:d.internetRevenue},{name:'AI infrastructure',color:'#63ff91',points:d.aiInfra},{name:'AI revenue',color:'#63ff91',points:d.aiRevenue}],year=>`${Math.round(year)}${year>2025?' · scenario':''}`,value=>`$${value.toFixed(2)}T / year`);
