...
function drawShare(scene){
 ...
 const shareBounds={left:m.l,right:W-m.r,top:m.t,bottom:m.t+ih};window.AIChartHover?.attach(svg,{bounds:shareBounds,keyboard:d.quarters.map((_,j)=>({x:m.l+(j+.5)*iw/9,y:m.t+ih/2})),get:point=>{const j=clamp(Math.floor((point.x-m.l)/iw*9),0,8),q=d.quarters[j];let bottom=0;const items=[];q.values.forEach((v,i)=>{if(v>=.05)items.push({label:d.groups[i].name,value:`${v.toFixed(1)}%`,color:d.groups[i].color,x:m.l+(j+.5)*iw/9,y:y(bottom+v/2)});bottom+=v;});return{title:q.label,x:m.l+(j+.5)*iw/9,items};}});
