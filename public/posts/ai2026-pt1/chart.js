...
function drawRevenue(scene){const {svg,W,H,small}=svgBase(scene),left=small?78:110,right=small?8:25,gap=small?18:50,pw=(W-left-right-gap)/2,top=45,bottom=35,rh=(H-top-bottom)/data.revenue.length,max=320;
 ...
 data.revenue.forEach((d,i)=>{const y=top+i*rh;if(panel===0){const nameLabel=label(svg,left-9,y+rh*.58,d.name,'end');if(d.name==='NVIDIA'){nameLabel.style.fill=colors.Chips;nameLabel.style.fontWeight='700';}}const value=d.values[panel];if(value===null){label(svg,x0+3,y+rh*.58,'·');return;}const bar=node('rect',{x:x0,y:y+rh*.15,width:0,height:rh*.66,fill:colors[d.category],'fill-opacity':.9,stroke:colors[d.category],'stroke-width':.5},svg);node('title',{},bar,`${d.name}, ${panel?'2026':'2024'}: approx. $${value}B`);const valueLabel=d.name==='NVIDIA'?label(svg,x0+Math.max(small?52:0,value/max*pw)-4,y+rh*.59,`~$${Math.round(value)}B`,'end','value'):null;bars.push({bar,panel,value,name:d.name,valueLabel});});}
...
function drawProfit(scene){const {svg,W,H,small}=svgBase(scene),margin=scene.dataset.kind==='margin',metric=margin?'margin':'profit',rows=data.profit.filter(d=>d.name!=='IBM'),left=small?82:130,right=small?55:100,top=32,bottom=36,iw=W-left-right,rh=(H-top-bottom)/rows.length,domain=margin?[0,80]:[0,290],x=v=>left+(v-domain[0])/(domain[1]-domain[0])*iw;
 ...
 const bars=[];rows.forEach((d,i)=>{const y=top+i*rh,val=d[metric],color=d.name==='NVIDIA'?'#63ff91':'#7293a9';const nameLabel=label(svg,left-9,y+rh*.58,d.name,'end');if(d.name==='NVIDIA'){nameLabel.style.fill=color;nameLabel.style.fontWeight='700';}if(d.name==='OpenAI'){label(svg,x(0)+4,y+rh*.58,'Negative; not public','start','value');return;}if(val===null){label(svg,x(0)+4,y+rh*.58,small?'Positive; n/a':'Positive; not public','start','value');return;}
...
function drawShare(scene){
 ...
 const future=node('rect',{y:m.t,height:ih,fill:'#b985ff','fill-opacity':.04,stroke:'#b985ff','stroke-dasharray':'5 6'},svg),open=label(svg,0,y(50),'Open','middle','series-label'),name=label(svg,0,y(35),'NVIDIA','middle','series-label');name.style.fill='#fff';open.style.fill='#b985ff';legend(scene,[]);
 return p=>{const {index,local}=passageStage(scene,p),extend=all?1:index?clamp(local/.6):0,count=9+4*extend,bw=iw/count,reveal=all||index?9:Math.max(1,Math.ceil(local/.8*9));for(const {b,j} of bars){b.setAttribute('x',m.l+j*bw+1);b.setAttribute('width',Math.max(0,bw-2));b.style.visibility=j<reveal?'visible':'hidden';}ticks.forEach((t,j)=>{t.setAttribute('x',m.l+(j+.5)*bw);t.style.visibility=j<reveal&&(!small||j%4===0)?'visible':'hidden';});future.setAttribute('x',m.l+9*bw+4);future.setAttribute('width',Math.max(0,4*extend*bw-4));future.style.visibility=extend>0?'visible':'hidden';open.setAttribute('x',m.l+(9+2*extend)*bw);open.style.opacity=extend;name.setAttribute('x',m.l+Math.min(reveal,9)*bw/2);scene.querySelector('.readout').textContent=index?'Future share is open':d.quarters[reveal-1].label;};
}
