...
function hoverRows(svg,bounds,rows){
 window.AIChartHover?.attach(svg,{bounds,keyboard:rows.map(r=>({x:r.x??(bounds.left+bounds.right)/2,y:r.y})),get:point=>{const row=rows.reduce((best,item)=>Math.abs(item.y-point.y)<Math.abs(best.y-point.y)?item:best,rows[0]);return {...row,x:row.x??point.x,guide:row.guide??'y'};}});
}
...
function drawRevenue(scene){const {svg,W,H,small}=svgBase(scene),left=small?78:110,right=small?8:25,gap=small?18:50,pw=(W-left-right-gap)/2,top=45,bottom=35,rh=(H-top-bottom)/data.revenue.length,max=320;
 ...
 const revenueBounds={left,right:W-right,top,bottom:H-bottom};window.AIChartHover?.attach(svg,{bounds:revenueBounds,keyboard:[0,1].flatMap(panel=>data.revenue.map((_,i)=>({x:left+panel*(pw+gap)+pw/2,y:top+(i+.5)*rh}))),get:point=>{const panel=point.x<left+pw+gap/2?0:1,i=clamp(Math.floor((point.y-top)/rh),0,data.revenue.length-1),d=data.revenue[i],value=d.values[panel],x0=left+panel*(pw+gap);return{title:panel?'Q1 2026':'Q1 2024',x:x0+(value??0)/max*pw,y:top+(i+.5)*rh,guide:'y',items:[{label:d.name,value:value===null?'No reported value':`≈ $${value.toFixed(1)}B`,color:colors[d.category],x:x0+(value??0)/max*pw,y:top+(i+.5)*rh}]};}});
...
function drawProfit(scene){const {svg,W,H,small}=svgBase(scene),margin=scene.dataset.kind==='margin',metric=margin?'margin':'profit',rows=data.profit.filter(d=>d.name!=='IBM'),left=small?82:130,right=small?55:100,top=32,bottom=36,iw=W-left-right,rh=(H-top-bottom)/rows.length,domain=margin?[0,80]:[0,290],x=v=>left+(v-domain[0])/(domain[1]-domain[0])*iw;
 ...
 hoverRows(svg,{left,right:W-right,top,bottom:H-bottom},rows.map((d,i)=>{const val=d[metric],color=d.name==='NVIDIA'?'#63ff91':'#7293a9',plotted=d.name!=='OpenAI'&&d.name!=='Anthropic'&&val!==null;return{title:margin?'Operating margin':'Annualized operating profit',x:plotted?x(val):x(0),y:top+(i+.5)*rh,items:[{label:d.name,value:plotted?(margin?`${val.toFixed(1)}%`:`${val<0?'−':''}$${Math.abs(val).toFixed(1)}B`):'Private company · not plotted',color,x:plotted?x(val):x(0),y:top+(i+.5)*rh}]};}));
