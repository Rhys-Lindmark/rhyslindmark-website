...
function draw(){const W=Math.max(280,svg.clientWidth),H=Math.max(240,svg.clientHeight),left=W<450?85:110,right=55,top=24,bottom=48,iw=W-left-right,rh=(H-top-bottom)/4,x=v=>left+v/24*iw;
 svg.replaceChildren();svg.setAttribute('viewBox',`0 0 ${W} ${H}`);node('title',{},'Estimated technology gap, 2025');
 ...
 const bar=node('rect',{x:left,y,width:0,height:h,fill:'#35e7ff','fill-opacity':.7});const value=text(left,y+h/2+4,`${r.gap} yrs`,{'text-anchor':'start',fill:'#fff'});
 const marker=node('line',{x1:x(r.prior),x2:x(r.prior),y1:y-5,y2:y+h+5,stroke:'#fff','stroke-width':2,'stroke-dasharray':'4 5'}),year=text(x(r.prior),y+h+20,String(r.year));return{bar,value,marker,year,width:r.gap/24*iw,left};});update();}
 function update(){const all=reduce.matches||document.body.classList.contains('all-mode'),p=all?1:clamp((90-host.getBoundingClientRect().top)/Math.max(1,host.offsetHeight-figure.offsetHeight));bars.forEach((r,i)=>{const t=all?1:clamp((p*4-i)/.8),visible=t>0?'visible':'hidden';r.bar.setAttribute('width',r.width*t);r.value.setAttribute('x',r.left+r.width*t+8);r.value.style.visibility=t===1?'visible':'hidden';r.marker.style.visibility=visible;r.year.style.visibility=visible;});}
