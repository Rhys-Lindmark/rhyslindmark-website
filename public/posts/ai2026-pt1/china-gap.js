...
function draw(){const W=Math.max(280,svg.clientWidth),H=Math.max(240,svg.clientHeight),left=W<450?85:110,right=55,top=24,bottom=48,iw=W-left-right,rh=(H-top-bottom)/4,x=v=>left+v/24*iw;
 ...
 const bar=node('rect',{x:left,y,width:0,height:h,fill:'#35e7ff','fill-opacity':.7});const value=text(left,y+h-5,'2026',{class:'gap-value','text-anchor':'end'});
 const marker=node('line',{x1:x(r.prior),x2:x(r.prior),y1:y-5,y2:y+h+5,stroke:'#fff','stroke-width':2,'stroke-dasharray':'4 5'}),year=text(x(r.prior),y+h+20,String(r.year));return{bar,value,marker,year,width:r.gap/24*iw,left,markerTop:y-5,markerBottom:y+h+5};});
 const bounds={left,right:W-right,top,bottom:H-bottom},rows=data.rows.map((r,i)=>({r,bar:bars[i].bar,y:top+(i+.43)*rh,x:x(r.gap)}));window.AIChartHover?.attach(svg,{bounds,keyboard:rows.map(row=>({x:left+row.r.gap/48*iw,y:row.y})),get:point=>{const row=rows.find(item=>{const bar=item.bar,bx=Number(bar.getAttribute('x')),by=Number(bar.getAttribute('y')),bw=Number(bar.getAttribute('width')),bh=Number(bar.getAttribute('height'));return bw>0&&point.x>=bx&&point.x<=bx+bw&&point.y>=by&&point.y<=by+bh;});if(!row)return null;const r=row.r;return{title:`${r.name} vs ${r.leader}`,x:row.x,y:row.y,guide:'y',items:[{label:'2026 estimate',value:`${r.gap} years behind`,color:'#35e7ff',x:row.x,y:row.y}]};}});
 update();}
 function update(force=false){const all=force===true||reduce.matches||document.body.classList.contains('all-mode'),p=all?1:clamp((90-host.getBoundingClientRect().top)/Math.max(1,host.offsetHeight-figure.offsetHeight));bars.forEach((r,i)=>{const historical=all?1:clamp((p/.4*4-i)/.8),t=all?1:clamp(((p-.4)/.6*4-i)/.8);r.bar.setAttribute('width',r.width*t);r.value.setAttribute('x',r.left+r.width*t-4);r.value.style.visibility=t===1?'visible':'hidden';r.marker.style.visibility=historical>0?'visible':'hidden';r.marker.setAttribute('y2',r.markerTop+(r.markerBottom-r.markerTop)*historical);r.year.style.opacity=historical;});}
 function request(){if(!frame)frame=requestAnimationFrame(()=>{frame=0;update();});}
 addEventListener('prepare-share-poster',event=>{if(event.detail.step===37)update(true);});
