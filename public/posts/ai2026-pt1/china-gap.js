...
function draw(){const W=Math.max(280,svg.clientWidth),H=Math.max(240,svg.clientHeight),left=W<450?85:110,right=55,top=24,bottom=48,iw=W-left-right,rh=(H-top-bottom)/4,x=v=>left+v/24*iw;
 ...
 const bounds={left,right:W-right,top,bottom:H-bottom},rows=data.rows.map((r,i)=>({r,y:top+(i+.43)*rh,x:x(r.gap)}));window.AIChartHover?.attach(svg,{bounds,keyboard:rows.map(row=>({x:row.x,y:row.y})),get:point=>{const row=rows.reduce((best,item)=>Math.abs(item.y-point.y)<Math.abs(best.y-point.y)?item:best,rows[0]),r=row.r;return{title:`${r.name} vs ${r.leader}`,x:row.x,y:row.y,guide:'y',items:[{label:'2026 estimate',value:`${r.gap} years behind`,color:'#35e7ff',x:row.x,y:row.y},{label:`${r.year} estimate`,value:`${r.prior} years behind`,color:'#fff',x:x(r.prior),y:row.y}]};}});
 update();}
