...
function attachHover(svg,scene,d,W,H,kind){
 ...
 if(scene.id==='omarchy'){
  const a={left:small?58:76,right:W-(small?20:28),top:20,bottom:H-65},x=scaler(d.x,a.left,a.right),y=scaler(d.y,a.bottom,a.top),rows=d.rectangles.map((r,i)=>({r,i,x:x(r.duration),y:y(r.monthlyCost)}));
  attach(a,rows,point=>{const hit=nearest(rows,point,e=>e.x,e=>e.y),p=state().reduced?1:ease((state().progress-hit.i*.33)/.4);if(p<=0)return null;return{title:hit.r.label,guide:false,items:[{label:'Duration',value:`${(hit.r.duration*p).toFixed(1)} months`,color:colors[hit.i],x:x(hit.r.duration*p),y:hit.y},{label:'Monthly cost',value:format(hit.r.monthlyCost,'dollars'),color:colors[hit.i]},{label:'Total',value:format(hit.r.total*p,'dollars'),color:colors[hit.i]}]}});return;
 }
